...
class SGFormerContrastive(nn.Module):
    def __init__(
        self,
        in_channels,
        hidden_channels,
        ...
        if aggregate == "add":
            self.phasing_embedding = nn.Linear(hidden_channels, out_channels)
            self.scaffold_embedding = nn.Linear(hidden_channels, out_channels)
        elif aggregate == "cat":
            self.phasing_embedding = nn.Linear(2 * hidden_channels, out_channels)
            self.scaffold_embedding = nn.Linear(2 * hidden_channels, out_channels)
        else:
        ...
        self.params2.extend(list(self.phasing_embedding.parameters()))
        self.params2.extend(list(self.scaffold_embedding.parameters()))
    def forward(self, x, edge_index):
        ...
        if self.use_graph:
            ...
            x = x1
        x1 = self.phasing_embedding(x)
        x2 = self.scaffold_embedding(x) 
        return x1, x2
